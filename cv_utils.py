...
def create_model(dataset, model_name="resnet18", device=torch.device("cpu"), **kwargs):
    ...
    # https://pytorch.org/vision/main/models/vgg.html
        ...
        model = torchvision.models.vgg11(weights="DEFAULT", **kwargs)
        if not isinstance(dataset, ImageNet):
            model.fc = nn.Linear(2048, dataset.n_class)
    # https://pytorch.org/vision/main/models/squeezenet.html
    elif model_name == "sqeezenet1_1":
        model = torchvision.models.squeezenet1_1(weights="DEFAULT", **kwargs)
        if not isinstance(dataset, ImageNet):
            model.fc = nn.Linear(2048, dataset.n_class)
    # https://pytorch.org/vision/main/models/swin_transformer.html
    elif model_name == "swin_s":
        model = torchvision.models.swin_s(weights="DEFAULT", **kwargs)
        if not isinstance(dataset, ImageNet):
            model.fc = nn.Linear(2048, dataset.n_class)
    # https://pytorch.org/vision/main/models/vision_transformer.html
    elif model_name == "vit_b_16":
        model = torchvision.models.vit_b_16(weights="DEFAULT", **kwargs)
        if not isinstance(dataset, ImageNet):
            model.fc = nn.Linear(2048, dataset.n_class)
    # https://pytorch.org/vision/main/models/generated/torchvision.models.resnext50_32x4d.html#torchvision.models.resnext50_32x4d
    elif model_name == "resnext50_32x4d":
        model = torchvision.models.resnext50_32x4d(weights="DEFAULT", **kwargs)
        if not isinstance(dataset, ImageNet):
            model.fc = nn.Linear(2048, dataset.n_class)
    else:
